...
<div class="space-y-6 max-w-4xl">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-semibold text-gray-900">Edit Penyakit</h1>
            <p class="mt-1 text-sm text-gray-600">Perbarui data penyakit beserta relasi komoditasnya.</p>
        </div>
    ...
    <form method="POST" action="{{ route('knowledge.penyakit.update', $penyakit) }}" class="space-y-6">
        @method('PUT')
        @csrf
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4 sm:p-6 space-y-5">
            ...
            <div>
                <label class="inline-flex items-center gap-2 text-sm font-medium text-gray-700 select-none cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $penyakit->is_active) ? 'checked' : '' }}
