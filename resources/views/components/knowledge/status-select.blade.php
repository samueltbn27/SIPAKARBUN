@props(['name' => 'status', 'value' => null, 'default' => 'draft', 'label' => 'Status', 'locked' => false])
<div>
    <label for="{{ $name }}" class="block text-sm font-medium text-gray-700">{{ $label }}</label>
    @if($locked)
        <input type="hidden" name="{{ $name }}" value="draft">
        <div class="mt-1 rounded-lg border border-[#dbece1] bg-[#f5fbf7] px-3 py-2 text-sm text-[#176b45]">Draft (ditinjau Operator UPTD sebelum dipublikasikan)</div>
    @else
